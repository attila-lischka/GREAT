...
from great.decoding.cvrp_decoder import CVRPDecoder
from great.envs.cvrp_env import CVRPEnv
...
class GREATRL_CVRP(nn.Module):
    def __init__(
        ...
        if matnet:
            ...
            self.encoder = RevMHAEncoder(
                ...
                n_heads=heads,
                embedding_dim=hidden_dim,
                input_dim=25,  # (2D EUC + theta) * 8 + demand
    ...
    def forward(self, data, return_length=False, augmentation_factor=1):
        ...
        ), "edge features need to be 2D (distance and demand)"
    ...
    def get_tour(self, data, augmentation_factor=1):
        embedding_list, dists_list, demands_list = [], [], []
        assert (
            data.edge_attr.dim() == 2
        ), "edge features need to be 2D (distance and demand)"
        ...
        if isinstance(
            self.encoder, GREATEncoder
        ):  # Pointerformer and matnet do not need rescaling
            coefficients = (
                torch.tensor(possible_factors).view(-1, 1, 1).to(embeddings.device)
            )
