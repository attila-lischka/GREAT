...
def rotate(nodes, rotation=0):
    assert rotation in list(range(16))
    nodes_dtype = nodes.dtype
    nodes_device = nodes.device
    flip = rotation // 8
    rotation = rotation % 8
    ...
    if flip == 1:
        x = nodes[:, 0]
        y = nodes[:, 1]
        nodes = torch.cat(((1 - x).unsqueeze(1), y.unsqueeze(1)), dim=1)

    radiants = rotation * math.pi / 4
    rotation_center = torch.tensor([0.5, 0.5], dtype=nodes_dtype).to(nodes_device)

    nodes = nodes - rotation_center

    rotation_matrix = torch.tensor(
        [[np.cos(radiants), -np.sin(radiants)], [np.sin(radiants), np.cos(radiants)]],
        dtype=nodes_dtype,
    ).to(nodes_device)

    nodes = torch.mm(nodes, rotation_matrix)

    nodes += rotation_center
