...
from great.dataset.dataset import GREATRLDataset, TestDataset
...
def eval(cfg: DictConfig):
    ...
    for dist in benchmark_distributions:
        ...
        if params["problem"] == "TSP":
            # get optimal TSP solutions that were computed with gurobi and saved
            gurobi_tours = get_gurobi_tours(
                data_dist=params["data_distribution"],
                tsp_size=params["instance_size"],
            )

        if params["data_distribution"] == "EUC" and params["problem"] == "TSP":
            # for EUC TSP we have special test data
        ...
        if tours_loadable(model_time_stamp + "_toursx8.txt"):
            RL_tours, elapsed_time = load_tours(
                model_time_stamp + "_toursx8.txt", params["problem"] == "TSP"
            )
            print(f"Elapsed time 8x RL : {elapsed_time:.4f} seconds")
        else:
            ...
            save_tours(RL_tours, elapsed_time, model_time_stamp + "_toursx8.txt")

        if tours_loadable(model_time_stamp + "_toursx1.txt"):
            RL_tours_1, elapsed_time = load_tours(
                model_time_stamp + "_toursx1.txt", params["problem"] == "TSP"
            )
            ...
            elapsed_time = end_time - start_time
            print(f"Elapsed time 1x RL : {elapsed_time:.4f} seconds")
            save_tours(RL_tours_1, elapsed_time, model_time_stamp + "_toursx1.txt")

        if tours_loadable(model_time_stamp + "_toursx32.txt"):
            RL_tours_32, elapsed_time = load_tours(
                model_time_stamp + "_toursx32.txt", params["problem"] == "TSP"
            )
            print(f"Elapsed time 32x RL : {elapsed_time:.4f} seconds")
        else:
            # evaluate the trained RL model using 32x augmentation factor
            start_time = time.time()
            with torch.no_grad():
                for i, vdata in enumerate(val_loader):
                    vdata.to(params["device"])
                    _, best_pi = model.get_tour(vdata, augmentation_factor=32)
                    RL_tours_32.extend(best_pi.cpu().numpy().tolist())
            end_time = time.time()
            # Calculate the elapsed time
            elapsed_time = end_time - start_time
            print(f"Elapsed time 32x RL : {elapsed_time:.4f} seconds")
            save_tours(RL_tours_32, elapsed_time, model_time_stamp + "_toursx32.txt")
        ...
        for i, data in enumerate(val_loader):
            ...
            if params["problem"] == "CVRP":
                ...
                prize = to_dense_adj(data.edge_index, data.batch, data.edge_attr)[
                    :, :, :, -1
                ]  # B x (N+1) x (N+1)
                prize = prize[
                    :, :1, :
        ...
        if params["problem"] == "TSP":
            ...
            end_time = time.time()
            elapsed_time = end_time - start_time
            print(f"Elapsed time nearest insertion : {elapsed_time:.4f} seconds")
        ...
        if params["problem"] == "TSP":
            ...
            end_time = time.time()
            elapsed_time = end_time - start_time
            print(f"Elapsed time farthest insertion : {elapsed_time:.4f} seconds")
        ...
        if params["problem"] == "TSP":
            if tours_loadable(
                "../TSP_RL/" + params["data_distribution"] + "_heuristic.txt"
            ):
                lkh_tours, elapsed_time = load_tours(
                    "../TSP_RL/" + params["data_distribution"] + "_heuristic.txt", True
                )
            else:
            ...
            if tours_loadable("../CVRP_RL/" + params["data_distribution"] + "_HGS.txt"):
                hgs_tours = load_hgs_tours(
                    "../CVRP_RL/" + params["data_distribution"] + "_HGS.txt"
                )
            else:
            ...
            if params["data_distribution"] == "EUC":
                if tours_loadable(
                    "../OP_RL/" + params["data_distribution"] + "_heuristic.txt"
                ):
                    ea4op_tours, elapsed_time = load_tours(
                        "../OP_RL/" + params["data_distribution"] + "_heuristic.txt"
                    )
                else:
                    start_time = time.time()
                    ea4op_tours = Parallel(n_jobs=1, verbose=10)(
                        delayed(get_EA4OP_results_op)(dist, prize, max_length, False)
        ...
        print("Computing distances of algorithms for " + params["data_distribution"])
        rl_dists = []
        rl_dists_1 = []
        rl_dists_32 = []
        ni_dists = []
        ...
        for i in range(len(dists)):
            ...
            if params["problem"] == "TSP":  # check TSP solution validity
                ...
                assert set(rl_32) == set(
                    range(params["instance_size"])
                ), "RL x32 solution invalid"
                assert set(ni) == set(
                    range(params["instance_size"])
                ), "NI solution invalid"
                assert set(fi) == set(
                ...
                assert set(gurobi) == set(
                    range(params["instance_size"])
                ), "Gurobi solution invalid"
                ...
                assert is_valid_cvrp(demands[i], capacity, rl_32)
                ...
                assert is_valid_op(rl_32, d, max_length)
            ...
            if params["problem"] in ["TSP", "CVRP"]:
                ...
                rl_dists_32.append(compute_tour_length(d, rl_32))
            elif params["problem"] == "OP":
                rl_dists.append(compute_collected_prize(rl, prizes[i]))
                rl_dists_1.append(compute_collected_prize(rl_1, prizes[i]))
                rl_dists_32.append(compute_collected_prize(rl_32, prizes[i]))
                op_dists.append(compute_collected_prize(op, prizes[i]))
                if params["data_distribution"] == "EUC":
                    ea4op_dists.append(compute_collected_prize(ea4op, prizes[i]))
            ...
            if params["problem"] == "TSP":
                ...
                gurobi_dists.append(compute_tour_length(d, gurobi))
        ...
        rl_dists_32 = round(sum(rl_dists_32) / len(rl_dists_32), 10)
        ...
        if params["problem"] == "TSP":
            ...
            gurobi_dists = round(sum(gurobi_dists) / len(gurobi_dists), 10)
        elif params["problem"] == "CVRP":
        ...
        if params["problem"] in ["TSP", "CVRP"]:
            ...
            print("Average length RL x32: " + str(rl_dists_32))
        elif params["problem"] == "OP":
            print("Average prize RL: " + str(rl_dists))
            print("Average prize RL x1: " + str(rl_dists_1))
            print("Average prize RL x32: " + str(rl_dists_32))
        ...
        if params["problem"] == "TSP":
            ...
            print("Average length Gurobi: " + str(gurobi_dists))
        elif params["problem"] == "CVRP":
            print("Average length HGS: " + str(hgs_dists))

        if params["problem"] == "TSP":
            print("Gaps relative to Gurobi: ")
            rl_gap = round(((rl_dists / gurobi_dists) - 1) * 100, 2)
            rl_1_gap = round(((rl_dists_1 / gurobi_dists) - 1) * 100, 2)
            rl_32_gap = round(((rl_dists_32 / gurobi_dists) - 1) * 100, 2)
            ni_gap = round(((ni_dists / gurobi_dists) - 1) * 100, 2)
            fi_gap = round(((fi_dists / gurobi_dists) - 1) * 100, 2)
            nn_gap = round(((nn_dists / gurobi_dists) - 1) * 100, 2)
            lkh_gap = round(((lkh_dists / gurobi_dists) - 1) * 100, 2)
        elif params["problem"] == "CVRP":
            print("Gaps relative to HGS: ")
            rl_gap = round(((rl_dists / hgs_dists) - 1) * 100, 2)
            rl_1_gap = round(((rl_dists_1 / hgs_dists) - 1) * 100, 2)
            rl_32_gap = round(((rl_dists_32 / hgs_dists) - 1) * 100, 2)
        elif params["problem"] == "OP":
            print("Gaps relative to Greedy: ")

            rl_gap = round(((op_dists / rl_dists) - 1) * 100, 2)
            rl_1_gap = round(((op_dists / rl_dists_1) - 1) * 100, 2)
            rl_32_gap = round(((op_dists / rl_dists_32) - 1) * 100, 2)
            if params["data_distribution"] == "EUC":
                ea4op_gap = round(((op_dists / ea4op_dists) - 1) * 100, 2)

        print("Average gap RL: " + str(rl_gap) + "%")
        print("Average gap RL x1: " + str(rl_1_gap) + "%")
        print("Average gap RL x32: " + str(rl_32_gap) + "%")
        if params["problem"] == "TSP":
            ...
            print("Average gap LKH: " + str(lkh_gap) + "%")
        if params["problem"] == "OP" and params["data_distribution"] == "EUC":
